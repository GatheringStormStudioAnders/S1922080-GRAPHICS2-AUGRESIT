...
void Shader::init(const std::string& vertFile, const std::string& fragFile, bool isGeo)
{
	if (!isGeo)
	{
	...
	else
	{
		program = glCreateProgram(); // create shader program (openGL saves as ref number)
		shaders[0] = CreateShader(LoadShader(vertFile), GL_VERTEX_SHADER); // create vertex shader
		shaders[1] = CreateShader(LoadShader("..\\res\\GeometryShader.geom"), GL_GEOMETRY_SHADER);
